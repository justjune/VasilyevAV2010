...
public:
	...
	MyTree *pnt[n];
	//Конструктор:
	MyTree(int k){ // к - количество уровней дерева
		int i;
		if(k==1)
		   for(i=0;i<n;i++) pnt[i]=NULL;  // Создать листья
		else
		   for(i=0;i<n;i++) pnt[i]=new MyTree(k-1); // Рекурсия - создать поддеревья для каждого потомка 
		cout<<"created "<<k<<" : "<<this<<endl;
		for(i=0;i<n;i++)
		   cout<<i<<": "<<pnt[i]<<endl;
		cout<<endl;
	}
