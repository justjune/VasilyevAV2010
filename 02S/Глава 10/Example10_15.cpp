/*
Пример класса для работы с полиномами 5 порядка 
*/
#include <iostream>
#include <cmath>
using namespace std;
...
double a[n+1];
//Перегрузка оператора "скобки" (т.е. ()): - вычисление значения полинома
double operator()(double z){
int i;
double s=0;
for(i=0;i<=n;i++) s+=a[i]*pow(z,i);
return s;}
//Перегрузка оператора "запятая" (т.е. ,): - вычисление производной порядка к
